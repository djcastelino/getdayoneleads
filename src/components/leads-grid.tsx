  ...
  return (
    <div className="space-y-4">
      <header className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-white">Leads Timeline</h2>
        <span className="text-xs uppercase tracking-wider text-slate-400">Newest first</span>
      </header>
  ...
  return (
    <>
      {Array.from({ length: 3 }).map((_, index) => (
        <div key={index} className="h-56 animate-pulse rounded-3xl border border-white/5 bg-white/5" />
      ))}
    </>
  );
}

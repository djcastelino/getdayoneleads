...
export function AlertsSidebar({ selectedSectors }: AlertsSidebarProps) {
  ...
  return (
    <aside className="flex h-fit flex-col gap-6 rounded-3xl border border-white/10 bg-slate-900/70 p-6" id="alerts">
      <header className="flex items-center gap-3">
        <div className="rounded-full border border-orange-400/40 bg-orange-500/10 p-2 text-orange-200">
          <Bell size={16} />
        </div>
        <div>
          <p className="text-xs uppercase tracking-[0.3rem] text-orange-300">Alerts & Upgrades</p>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/5 p-4">
        <h4 className="flex items-center gap-2 text-sm font-semibold text-white">
          <Flame size={16} className="text-orange-300" /> Saved searches
        </h4>
        <ul className="mt-3 space-y-3 text-sm text-slate-200">
          {selectedSectors.map((sector) => (
            <li key={sector} className="flex items-center justify-between">
              <span>{sector}</span>
              <button type="button" className="text-xs uppercase tracking-wider text-teal-200">
                Manage
              </button>
            </li>
          ))}
        ...
        <button
          type="button"
          className="mt-4 inline-flex items-center gap-2 text-xs uppercase tracking-wider text-teal-200"
        >
    ...
    <div className="flex items-start justify-between gap-3 rounded-2xl border border-white/10 bg-white/5 p-4">
      ...
      <button
        ...
        className={`relative inline-flex h-6 w-11 items-center rounded-full border border-white/10 transition ${
          enabled ? "bg-teal-500/80" : "bg-slate-800"
        }`}

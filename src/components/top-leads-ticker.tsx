...
export function TopLeadsTicker({ leads, isLoading }: TopLeadsTickerProps) {
  ...
  return (
    <div className="relative flex h-full flex-col justify-between overflow-hidden rounded-3xl border border-white/10 bg-slate-900/50 p-6">
      <header>
        <p className="text-xs uppercase tracking-[0.3rem] text-orange-300">Radar Highlights</p>
        <h2 className="mt-3 text-xl font-semibold text-white">Top Awards by Value</h2>
      </header>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {activeLead ? (
              <>
                <div>
                  <span className="inline-flex items-center gap-2 rounded-full border border-orange-400/40 bg-orange-500/10 px-3 py-1 text-xs font-medium text-orange-200">
                ...
                <div className="flex items-center justify-between text-sm text-slate-200/80">
                  <div>
                    ...
                    <p className="text-base font-semibold text-white">{activeLead.value}</p>
                  </div>
        ...
        <div className="flex gap-1">
          {leads.map((lead, idx) => (
            <button
              key={lead.awardId}
              type="button"
              className={`h-1.5 w-8 rounded-full transition ${idx === index ? "bg-orange-400" : "bg-white/20"}`}
      ...
      {isLoading && (
        <div className="absolute inset-0 animate-pulse rounded-3xl bg-slate-950/20" aria-hidden />
      )}
    </div>
  );
}
